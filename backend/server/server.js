...
const frontendPath = path.join(__dirname, "../dist"); // make sure your React build is in ../dist
...
io.on("connection", (socket) => {
  ...
  socket.on("moduleOpened", ({ roomCode, moduleId }) => {
    try {
      ...
      // ** Always get the next question from backend ** (no random)
      const lastIndex = room.lastQuestion?.questionIndex ?? -1;
      const nextIndex = (lastIndex + 1) % moduleQuestions.length;
      const q = moduleQuestions[nextIndex];

      room.lastQuestion = { moduleId, questionIndex: nextIndex };
